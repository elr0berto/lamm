...
export class GameLeft extends Scene {
    ...
    private handleDepart = (data: { id: string; side: string }) => {
        if (data.side !== 'left') return; // only remove if departing from here
    ...
    create() {
        ...
        // Init player manager (idempotent)
        ...
        // Spawn any players currently assigned to left side
        playerManager.getPlayersForSide('left').forEach(p => this.spawnPlayer(p));
